...
@router.post("/workflows/execute", response_model=WorkflowExecuteResponse, status_code=status.HTTP_201_CREATED)
async def execute_workflow_endpoint(
    ...
    # Trigger Celery task
    execute_workflow.delay(job.id, workflow_request.workflow_type, workflow_request.input_data)
...
async def get_metrics(
    ...
    total_jobs = db.query(func.count(WorkerJob.id)).filter(
        WorkerJob.organization_id == current_user.organization_id
    ).scalar()
    
    pending_jobs = db.query(func.count(WorkerJob.id)).filter(
        WorkerJob.organization_id == current_user.organization_id,
        WorkerJob.status == "pending"
    ).scalar()
    
    running_jobs = db.query(func.count(WorkerJob.id)).filter(
        WorkerJob.organization_id == current_user.organization_id,
        WorkerJob.status == "running"
    ).scalar()
    
    completed_jobs = db.query(func.count(WorkerJob.id)).filter(
        WorkerJob.organization_id == current_user.organization_id,
        WorkerJob.status == "completed"
    ).scalar()
    
    failed_jobs = db.query(func.count(WorkerJob.id)).filter(
        WorkerJob.organization_id == current_user.organization_id,
