...
from shared.middleware import CurrentUser, require_org_admin
from shared.utils import get_db
from sqlalchemy.orm import Session

from .models import Theme
from .schemas import MessageResponse, ThemeResponse, ThemeUpdate
...
async def get_theme(org_id: int, db: Session = Depends(get_db)):
    """Get organization theme (public endpoint)"""
    theme = (
        db.query(Theme)
        .filter(Theme.organization_id == org_id, Theme.is_active.is_(True))
        .first()
    )
...
async def update_theme(
    ...
    if not theme:
        # Create new theme
        theme = Theme(organization_id=org_id, name=theme_update.name or "Default Theme")
        db.add(theme)
...
async def generate_css(org_id: int, db: Session = Depends(get_db)):
